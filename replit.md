...
- **Two-source read pipeline.** `useGutenbergMatch` runs first; if it returns nothing, `useArchiveMatch` fires (lazy, gated by `enabled`). The unified `readSource` on book detail picks Gutenberg first, Archive second, and exposes a primary format hint: **EPUB-primary for Gutenberg** (clean reflowable text), **PDF-primary for Internet Archive** (their PDFs are real scans with OCR text; their auto-generated EPUBs are frequently malformed image-only files). The "via Project Gutenberg" / "via Internet Archive" tag below the buttons makes the source explicit.
...
- **First-party file proxy.** Browser CORS blocks Gutenberg/Archive directly, so all downloads route through our own `/api/proxy/epub?url=…` route. Allowlist: `www.gutenberg.org`, `gutenberg.org`, `archive.org`, and any `*.archive.org` subdomain (Archive download URLs redirect to dynamically-named storage hosts like `ia801505.us.archive.org`). Each redirect hop is re-validated against the allowlist (no SSRF). The proxy streams and TanStack Query caches the ArrayBuffer (`gcTime: 1h`, `staleTime: Infinity`) so page turns never refetch.
